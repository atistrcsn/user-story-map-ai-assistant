...
*   **Surgical Precision Mandate:** You are strictly forbidden from making any changes to the code that are not directly and explicitly required to fulfill the user's immediate request. This prohibits "corrective" changes to code outside the task's scope, such as altering configuration strings (e.g., model names), reformatting unrelated code, or renaming variables. Assume all out-of-scope code is correct and intentional.
