...
### Backend Architecture: Building Services with Go

This guide provides the decision framework for building backend services using Go (Golang). Go is the default and primary choice for all backend development due to its simplicity, performance, and first-class concurrency support.

#### Core Philosophy: Simplicity and Performance

*   **Simplicity:** Go's design favors clarity and readability. We prefer using the robust standard library over large, complex frameworks wherever possible.
*   **Performance:** As a compiled language, Go offers exceptional performance with a low memory footprint, making it ideal for high-throughput microservices and infrastructure tooling.
*   **Concurrency:** Goroutines and channels are fundamental language features, making it straightforward to build highly concurrent systems.

#### Web Frameworks and Routing

*   **Default Choice: Standard Library (`net/http`)**
    *   **Use Case:** For most services, the `net/http` package is powerful, stable, and sufficient. It encourages a deep understanding of HTTP and avoids unnecessary abstractions.
    *   **Best Practice:** Combine `net/http` with a lightweight routing library like **`chi`** to add middleware support and convenient URL parameter handling without sacrificing the standard library's philosophy.

*   **High-Feature Alternative: `Gin`**
    *   **Use Case:** For complex APIs requiring a larger feature set out-of-the-box (e.g., advanced middleware, rendering, validation). Gin offers excellent performance and a rich ecosystem. Use it when development speed for a feature-heavy service is the top priority.

#### Decision Rubric

| Consideration | Choose `net/http` + `chi` | Choose `Gin` |
| :--- | :--- | :--- |
| **Primary Goal** | Simplicity, maintainability, minimal dependencies | Rapid development of feature-rich APIs |
| **Project Type** | Microservices, infrastructure, performance-critical APIs | Complex monolithic APIs, internal dashboards |
| **Architectural Style** | Explicit, idiomatic Go, composable | "Batteries-included," structured framework |

</details>

<details>
<summary>TECH_GUIDE:LOCAL_DEVELOPMENT_SETUP</summary>
### Local Development: A High-Velocity, Hot-Reload Setup for Go

The goal is a seamless "inner loop" where code changes are reflected instantly.

*   **Hot-Reloading:**
    *   **Tooling:** Use **`Air`** for live reloading. It watches for file changes (`.go` files) and automatically recompiles and restarts the server, providing instant feedback during development.

*   **Unified Local Environment (Mandatory):**
    *   **Containerization (`docker-compose`):** This is the **best practice**. Create a `docker-compose.yml` file to define and run your entire local stack: the Go application container and any stateful services like a local Postgres or Redis container.
    *   **Benefit:** A single command (`docker-compose up`) starts everything. Every developer gets the exact same setup, eliminating "it works on my machine" issues and perfectly mirroring the container-based production environment.

</details>

<details>
<summary>TECH_GUIDE:CODE_QUALITY_AND_DEPENDENCIES</summary>
### Code Quality & Dependency Management in Go

These are mandatory for all projects to ensure code is clean, correct, and reproducible.

*   **Code Style & Linting (Enforced in CI):**
    *   **Formatting:** **`gofmt`** is the non-negotiable, official formatting tool. All code must be formatted with `gofmt` before being committed.
    *   **Linting:** Use **`staticcheck`** as the primary linter. It provides an extensive set of checks for bugs, performance issues, and stylistic errors, going far beyond the standard `go vet`.

*   **Dependency Management:**
    *   **Tooling:** All projects **must** use **Go Modules**. The `go.mod` file defines the project's dependencies, and the `go.sum` file contains the checksums to ensure reproducible builds. These files must be committed to the repository.

*   **Language-Specific Ecosystem Enhancements:**
    *   **Configuration Management:** Use **`Viper`** for handling application configuration. It supports reading from files (JSON, TOML, YAML), environment variables, and remote config systems, providing a unified solution.
    *   **CLI Tooling:** Use **`Cobra`** to build powerful, modern CLI applications. It is the foundation of many popular tools like `kubectl` and `hugo` and provides a robust framework for commands and flags.

</details>

<details>
<summary>TECH_GUIDE:TESTING_STRATEGY</summary>
### The Testing Pyramid: A Strategy for Confidence in Go
...
*   **Level 1: Unit Tests (Most Numerous):**
    ...
    *   **Tools:** Use the built-in **`testing`** package. For more expressive assertions and mocking, use the **`testify`** suite (`testify/assert`, `testify/require`, `testify/mock`).

*   **Level 2: Integration Tests:**
    *   **Goal:** Test the interaction between services (e.g., your Go service to a database).
    *   **Environment:** Run these against the stateful services defined in your `docker-compose.yml` (e.g., a real Postgres instance) within your CI pipeline.

*   **Level 3: End-to-End (E2E) Tests (Least Numerous):**
    *   **Goal:** Test a full API workflow or user journey.
    *   **Tools:** For API testing, use Go's built-in **`net/http/httptest`** package to make real HTTP requests to your running service and verify the responses. For browser-based E2E tests, standard tools like **Playwright** or **Cypress** can be used to test the system as a whole.

</details>

<details>
<summary>TECH_GUIDE:PRODUCTION_READINESS</summary>
### Enterprise-Grade Production Readiness for Go Services

This section covers the critical pillars of security, testing, and observability that ensure a Go application is truly production-ready, secure, and maintainable.
...
*   **Recommendations:**
    *   **Google Cloud Identity Platform (GCIP) / Firebase Auth:** The default choice for seamless integration with Google Cloud.
    *   **Auth0 / Okta:** Excellent, vendor-agnostic alternatives for complex enterprise environments.

#### 2. Deep Observability: Beyond Logs

To understand and debug a distributed system, you need more than just logs.

*   **Structured Logging:** All services **must** log in JSON format. Use the standard library's **`log/slog`** package (available since Go 1.21) as the default for structured, leveled logging.
*   **Distributed Tracing:** Implement **OpenTelemetry for Go**. This is the industry standard for tracing requests as they flow through your services, invaluable for pinpointing bottlenecks and errors.
*   **Metrics & Alerting:** Use a library like **Prometheus**'s official Go client to expose key application metrics. In Cloud Monitoring, define and monitor Service Level Objectives (SLOs) and create alerts for critical thresholds (e.g., p99 latency, error rate).
...
*   **Automated Vulnerability Scanning (CI/CD):**
    *   **Dependency Scanning:** The CI pipeline **must** include a step to run **`govulncheck`**. This is the official Go tool for analyzing your codebase to find known vulnerabilities in your dependencies.
